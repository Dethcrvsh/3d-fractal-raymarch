...
void sphereFold(inout vec4 z, inout float dz, float minRadius, float fixedRadius) {
    float r = dot(z,z);
    if (r<minRadius) { 
...
float estimate_distance_color(const vec3 p)
{
    ...
    {
		box_fold(z, 1);
		sphereFold(z, dr, 0.01, 1);
	    z = z * in_RotTest2;
	    z = z * in_RotTest1;
		sphereFold(z, dr, 0.01, 2);
...
vec4 get_hit_color(const int iter, const vec3 curr_pos, const vec3 dir)
{
    ...
    // Calculate color
	const float STEP_LEN = MIN_DIST/2;
    ...
    // Normals
    vec3 pos = curr_pos - dir*MIN_DIST; // Backtrack to get better normals
